 ...
 * crafts health potions at the alchemist, and deposits them back to bank.
 * @module health-potion-loop
 */
...
// Items needed for health potions
const REQUIRED_MATERIALS = [
  { code: 'nettle_leaf', quantity: 20 }, // 2 nettle_leaf per potion × 10 potions
  { code: 'sunflower', quantity: 10 },   // 1 sunflower per potion × 10 potions
  { code: 'sap', quantity: 10 }          // 1 sap per potion × 10 potions
];
...
// Health potion recipe code and quantity
const HEALTH_POTION_CODE = 'health_potion';
...
/**
 * Perform one complete health potion crafting cycle
 * @async
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
async function healthPotionCraftingCycle() {
  try {
    console.log('Starting health potion crafting cycle...');
    ...
    if (!potionsInInventory) {
      console.error('Failed to find crafted health potions in inventory!');
      return false;
    }
    
    console.log(`Successfully crafted health potions, found ${potionsInInventory.quantity} x ${HEALTH_POTION_CODE} in inventory`);
    ...
    console.log('Health potion crafting cycle completed successfully!');
    return true;
  } catch (error) {
    console.error('Health potion crafting cycle failed:', error.message);
    return false;
  }
}

/**
 * Main function that runs the health potion crafting loop
 * @async
...
async function main() {
  ...
  console.log(`Starting health potion crafting loop for character: ${characterName}`);
  ...
  const cycleWrapper = async () => {
    return await healthPotionCraftingCycle();
  };
...
/**
 * Module exports
 * @exports health-potion-loop
 */
module.exports = {
  healthPotionCraftingCycle
};