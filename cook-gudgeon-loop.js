/**
 * @fileoverview Script to create a loop that withdraws gudgeon from bank,
 * cooks it at the cook station, and deposits the cooked gudgeon back to bank.
 * @module cook-gudgeon-loop
 */
...
// Items needed for cooking gudgeon
const REQUIRED_MATERIALS = [
  { code: 'gudgeon', quantity: 100 } // Updated material
];
...
// Cooked gudgeon recipe code and quantity
const COOKED_GUDGEON_CODE = 'cooked_gudgeon'; // Updated product
const COOKED_GUDGEON_QUANTITY = 100; // Assuming same quantity
...
/**
 * Perform one complete gudgeon cooking cycle
 * @async
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
async function cookGudgeonCycle() {
  try {
    console.log('Starting gudgeon cooking cycle...');
    ...
    // 5. Cook gudgeon
    await waitForCooldown();
    console.log(`Cooking ${COOKED_GUDGEON_QUANTITY} x ${COOKED_GUDGEON_CODE}...`);
    await craftingAction(COOKED_GUDGEON_CODE, COOKED_GUDGEON_QUANTITY);
    console.log(`Successfully cooked ${COOKED_GUDGEON_QUANTITY} x ${COOKED_GUDGEON_CODE}`);
    
    // 6. Check that cooked gudgeon was created
    const cookedGudgeonInInventory = await checkInventoryForItem(COOKED_GUDGEON_CODE);
    if (!cookedGudgeonInInventory) {
      console.error('Failed to find cooked gudgeon in inventory!');
      return false;
    }
    
    console.log(`Successfully cooked gudgeon, found ${cookedGudgeonInInventory.quantity} x ${COOKED_GUDGEON_CODE} in inventory`);
    
    // 7. Move back to bank to deposit cooked gudgeon
    await moveToLocation(BANK_COORDS.x, BANK_COORDS.y);
    ...
    console.log('Gudgeon cooking cycle completed successfully!');
    return true;
  } catch (error) {
    console.error('Gudgeon cooking cycle failed:', error.message);
...
/**
 * Main function that runs the gudgeon cooking loop
 * @async
...
async function main() {
  ...
  console.log(`Starting gudgeon cooking loop for character: ${characterName}`);
...
if (require.main === module) {
  main().catch(error => {
      console.error("Unhandled error in main gudgeon cooking loop:", error);
      process.exit(1);
  });
}
