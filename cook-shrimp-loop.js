/**
 * @fileoverview Script to create a loop that withdraws shrimp from bank,
 * cooks it at the cook station, and deposits the cooked shrimp back to bank.
 * @module cook-shrimp-loop
 */
...
// Items needed for cooking shrimp
const REQUIRED_MATERIALS = [
  { code: 'shrimp', quantity: 100 } // Updated material
];
...
// Cooked shrimp recipe code and quantity
const COOKED_SHRIMP_CODE = 'cooked_shrimp'; // Updated product
const COOKED_SHRIMP_QUANTITY = 100; // Assuming same quantity
...
/**
 * Perform one complete shrimp cooking cycle
 * @async
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
async function cookShrimpCycle() {
  try {
    console.log('Starting shrimp cooking cycle...');
    ...
    // 5. Cook shrimp
    await waitForCooldown();
    console.log(`Cooking ${COOKED_SHRIMP_QUANTITY} x ${COOKED_SHRIMP_CODE}...`);
    await craftingAction(COOKED_SHRIMP_CODE, COOKED_SHRIMP_QUANTITY);
    console.log(`Successfully cooked ${COOKED_SHRIMP_QUANTITY} x ${COOKED_SHRIMP_CODE}`);
    
    // 6. Check that cooked shrimp was created
    const cookedShrimpInInventory = await checkInventoryForItem(COOKED_SHRIMP_CODE);
    if (!cookedShrimpInInventory) {
      console.error('Failed to find cooked shrimp in inventory!');
      return false;
    }
    
    console.log(`Successfully cooked shrimp, found ${cookedShrimpInInventory.quantity} x ${COOKED_SHRIMP_CODE} in inventory`);
    
    // 7. Move back to bank to deposit cooked shrimp
    await moveToLocation(BANK_COORDS.x, BANK_COORDS.y);
    ...
    console.log('Shrimp cooking cycle completed successfully!');
    return true;
  } catch (error) {
    console.error('Shrimp cooking cycle failed:', error.message);
...
/**
 * Main function that runs the shrimp cooking loop
 * @async
...
async function main() {
  ...
  console.log(`Starting shrimp cooking loop for character: ${characterName}`);
...
if (require.main === module) {
  main().catch(error => {
      console.error("Unhandled error in main shrimp cooking loop:", error);
      process.exit(1);
  });
}
