/**
 * @fileoverview Automated trout fishing bot that catches fish and deposits at bank when inventory is full.
 * Use node trout-harvesting-loop.js in the terminal to execute the script.
 * @module TroutHarvestingLoop
 */
...
/**
 * Class representing an automated trout fishing loop.
 * @extends BaseLoop
 */
class TroutHarvestingLoop extends BaseLoop {
  /**
  ...
  /**
   * Perform the trout fishing action and handle results.
  ...
  async fish() {
    try {
      ...
      console.log(`Starting trout fishing at coordinates (${beforeDetails.x}, ${beforeDetails.y})...`);
      ...
      await db.query(
        ...
        [
          this.characterName,
          'action/trout_fishing_start',
          beforeDetails.x || 0,
          beforeDetails.y || 0,
          { message: 'Starting trout fishing action' }
        ]
      );
      ...
      if (result && result.resources) {
        ...
        console.log(`Caught ${result.resources.length} trout (total: ${this.resourceCount})`);
      }
      ...
      if (error.message.includes('inventory is full')) {
        console.log('Inventory is full. Proceeding to deposit trout...');
        await this.depositFish();
        return null;
      } else if (error.message.includes('No resource on this map') || 
                 error.message.includes('Resource not found')) {
        console.log('No trout found. Will try again...');
        return null;
      } else {
  ...
  /**
   * Main loop that continuously fishes for trout and manages inventory.
  ...
  async runLoop() {
    try {
      console.log(`Initializing trout fishing loop at (${this.harvestCoords.x}, ${this.harvestCoords.y})...`);
      ...
      while (true) {
        ...
        if (await checkInventory(details)) {
          console.log('Inventory full, depositing trout...');
          await this.depositFish();
          continue;
        }
        ...
        console.log(`Completed loop #${this.loopCount}. Caught ${this.resourceCount} trout in total.`);
        
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    } catch (error) {
      console.error('Fatal error in trout fishing loop:', error.message);
      throw error;
    }
  }
  ...
  static async main() {
    ...
    try {
      process.on('SIGINT', async () => {
        ...
        console.log(`Caught ${loop.resourceCount} trout in total across ${loop.loopCount} loops.`);
    ...
    try {
      ...
      console.error('Trout fishing loop failed:', error.message);
      try {
        process.exit(1);
      } catch (e) {
...
if (require.main === module) {
  TroutHarvestingLoop.main().catch(error => {
    console.error('Unhandled error in trout fishing loop:', error);
    try {
      process.exit(1);
    } catch (e) {
      console.error('Failed to exit process:', e.message);
...
/**
 * Export the TroutHarvestingLoop class
 * @exports TroutHarvestingLoop
 */
module.exports = TroutHarvestingLoop;
