 ...
 * crafts leather boots at the armorsmith, and deposits them back to bank.
 * @module leather-boots-crafting-loop
 */
...
// Items needed for leather boots
const REQUIRED_MATERIALS = [
  { code: 'cowhide', quantity: 60 },
  { code: 'ash_plank', quantity: 60 }
];

// Armorsmith coordinates
const ARMORSMITH_COORDS = { x: 3, y: 1 };
// Bank coordinates
const BANK_COORDS = { x: 4, y: 1 };
// Leather boots recipe code and quantity
const LEATHER_BOOTS_CODE = 'leather_boots';
const LEATHER_BOOTS_QUANTITY = 15;
...
/**
 * Perform one complete leather boots crafting cycle
 * @async
 * @param {boolean} [shouldRecycle=true] - Whether to perform the recycling step
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
async function leatherBootsCraftingCycle(characterName = config.character, shouldRecycle = true) {
  try {
    console.log(`Starting leather boots crafting cycle... (Recycle: ${shouldRecycle})`);
    ...
    // 4. Move to armorsmith
    await moveToLocation(ARMORSMITH_COORDS.x, ARMORSMITH_COORDS.y);
    
    // 5. Craft leather boots
    await waitForCooldown();
    console.log(`Crafting ${LEATHER_BOOTS_QUANTITY} x ${LEATHER_BOOTS_CODE}...`);
    await craftingAction(LEATHER_BOOTS_CODE, LEATHER_BOOTS_QUANTITY);
    console.log(`Successfully crafted ${LEATHER_BOOTS_QUANTITY} x ${LEATHER_BOOTS_CODE}`);
    
    // 6. Check that leather boots were created
    const bootsInInventory = await checkInventoryForItem(LEATHER_BOOTS_CODE);
    if (!bootsInInventory) {
      console.error('Failed to find crafted leather boots in inventory!');
      return false;
    }
    
    console.log(`Successfully crafted leather boots, found ${bootsInInventory.quantity} x ${LEATHER_BOOTS_CODE} in inventory`);

    // 7. Recycle the crafted items (conditionally)
    if (shouldRecycle) {
      await waitForCooldown();
      console.log(`Recycling ${bootsInInventory.quantity} x ${LEATHER_BOOTS_CODE}...`);
      // Assuming recyclingAction uses config.character if not provided
      await recyclingAction(LEATHER_BOOTS_CODE, bootsInInventory.quantity, characterName);
    ...
    console.log('Leather boots crafting cycle completed successfully!');
    return true;
  } catch (error) {
    console.error('Leather boots crafting cycle failed:', error.message);
    return false;
  }
}

/**
 * Main function that runs the leather boots crafting loop
 * @async
...
async function main() {
  ...
  console.log(`Starting leather boots crafting loop for character: ${characterName}`);
  ...
  const cycleWrapper = async () => {
    ...
    return await leatherBootsCraftingCycle(characterName, shouldRecycle);
  };
...
/**
 * Module exports
 * @exports leather-boots-crafting-loop
 */
module.exports = {
  leatherBootsCraftingCycle
};
