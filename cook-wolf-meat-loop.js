/**
 * @fileoverview Script to create a loop that withdraws raw wolf meat from bank,
 * cooks it at the cook station, and deposits the cooked meat back to bank.
 * @module cook-wolf-meat-loop
 */
...
// Items needed for cooking wolf meat
const REQUIRED_MATERIALS = [
  { code: 'raw_wolf_meat', quantity: 100 }
];
...
// Cooked wolf meat recipe code and quantity
const COOKED_MEAT_CODE = 'cooked_wolf_meat';
const COOKED_MEAT_QUANTITY = 100;
...
/**
 * Perform one complete wolf meat cooking cycle
 * @async
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
async function cookWolfMeatCycle() {
  try {
    console.log('Starting wolf meat cooking cycle...');
    ...
    // 5. Cook wolf meat
    await waitForCooldown();
    console.log(`Cooking ${COOKED_MEAT_QUANTITY} x ${COOKED_MEAT_CODE}...`);
    await craftingAction(COOKED_MEAT_CODE, COOKED_MEAT_QUANTITY);
    console.log(`Successfully cooked ${COOKED_MEAT_QUANTITY} x ${COOKED_MEAT_CODE}`);
    
    // 6. Check that cooked meat was created
    const cookedMeatInInventory = await checkInventoryForItem(COOKED_MEAT_CODE);
    if (!cookedMeatInInventory) {
      console.error('Failed to find cooked wolf meat in inventory!');
      return false;
    }
    
    console.log(`Successfully cooked wolf meat, found ${cookedMeatInInventory.quantity} x ${COOKED_MEAT_CODE} in inventory`);
    
    // 7. Move back to bank to deposit cooked meat
    await moveToLocation(BANK_COORDS.x, BANK_COORDS.y);
    ...
    console.log('Wolf meat cooking cycle completed successfully!');
    return true;
  } catch (error) {
    console.error('Wolf meat cooking cycle failed:', error.message);
    return false;
  }
}

/**
 * Main function that runs the wolf meat cooking loop
 * @async
...
async function main() {
  ...
  console.log(`Starting wolf meat cooking loop for character: ${characterName}`);
  ...
  const cycleWrapper = async () => {
    return await cookWolfMeatCycle();
  };
