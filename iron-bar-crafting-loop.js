/**
 * @fileoverview Script to create a loop that withdraws iron ore from bank,
 * crafts iron bars at the forge, and deposits them back to bank.
 * @module iron-bar-crafting-loop
 */
...
// Materials needed for iron bars
...
// Iron bar recipe code and quantity
const IRON_BAR_CODE = 'iron';
const IRON_BAR_QUANTITY = 10;
...
/**
 * Perform one complete iron bar crafting cycle
 * @async
 * @param {string} [characterName] - Optional character name
 * @param {boolean} [shouldRecycle=true] - Whether to perform the recycling step
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
async function ironBarCraftingCycle(characterName = config.character, shouldRecycle = true) {
  try {
    console.log(`Starting iron bar crafting cycle... (Recycle: ${shouldRecycle})`);

    // 1. First check current inventory for iron ore
    console.log('Checking current inventory for iron ore...');
    ...
    if (needBank) {
      ...
      console.log('Need to withdraw iron ore, moving to bank...');
    ...
    // 5. Craft iron bars
    await waitForCooldown(characterName);
    console.log(`Crafting ${IRON_BAR_QUANTITY} x ${IRON_BAR_CODE}...`);
    await craftingAction(IRON_BAR_CODE, IRON_BAR_QUANTITY, null, characterName);
    console.log(`Successfully crafted ${IRON_BAR_QUANTITY} x ${IRON_BAR_CODE}`);
    
    // 6. Check that iron bars were created
    const ironBarInInventory = await checkInventoryForItem(IRON_BAR_CODE, characterName);
    if (!ironBarInInventory) {
      console.error('Failed to find crafted iron bars in inventory!');
      return false;
    }
    
    console.log(`Successfully crafted iron bars, found ${ironBarInInventory.quantity} x ${IRON_BAR_CODE} in inventory`);

    // 7. Recycle the crafted items (conditionally)
    if (shouldRecycle) {
      await waitForCooldown(characterName);
      console.log(`Recycling ${ironBarInInventory.quantity} x ${IRON_BAR_CODE}...`);
      await recyclingAction(IRON_BAR_CODE, ironBarInInventory.quantity, characterName);
    ...
    const remainingBars = await checkInventoryForItem(IRON_BAR_CODE, characterName);
    if (remainingBars && remainingBars.quantity > 0) {
        await waitForCooldown(characterName);
        await depositToBank(IRON_BAR_CODE, remainingBars.quantity, characterName);
        console.log(`Successfully deposited ${remainingBars.quantity} x ${IRON_BAR_CODE}`);
    } else {
        console.log('No iron bars left to deposit after recycling.');
    ...
    console.log('Iron bar crafting cycle completed successfully!');
    return true;
  } catch (error) {
    console.error('Iron bar crafting cycle failed:', error.message);
...
/**
 * Main function that runs the iron bar crafting loop
 * @async
...
async function main() {
  ...
  console.log(`Starting iron bar crafting loop for character: ${characterName}`);
  ...
  const cycleWrapper = async () => {
    ...
    return await ironBarCraftingCycle(characterName, shouldRecycle);
  };
...
/**
 * Module exports
 * @exports iron-bar-crafting-loop
 */
module.exports = {
  ironBarCraftingCycle
};
